...
- **Apple account plumbing is already wired.** `ASC_API_KEY_ID`, `ASC_API_ISSUER_ID`, and `ASC_API_KEY_CONTENT` are configured as repository secrets, and `testflight.yml` provisions certificates and profiles through the App Store Connect API on the self-hosted Mac. The archive/upload path is unexercised pending the toolchain gate above, not pending credentials.
...
- **The autonomous idea generator was failing in practice on the reference Mac Tank** because `trigger_autonomous_idea` shared the same 45-second `NOBS_OLLAMA_TIMEOUT_SECONDS` as interactive `/chat` and `/agent/tasks`, and the local Tank error log recorded 188 `AgentModelError`s caused by `httpx.ReadTimeout` even with Ollama, `qwen3:8b`, and the Tank API all verified healthy. `trigger_autonomous_idea` and `process_overnight_task` (the two scheduler-triggered agent turns nobody is synchronously waiting on) now use a separate `NOBS_AGENT_BACKGROUND_TIMEOUT_SECONDS` (default 120, deterministic tests in `tests/test_overnight_queue.py`), while interactive routes keep the fast 45-second default so a stuck model still fails a live chat quickly. This is untested against a live Ollama instance — treat scheduled idea generation as improved but not re-verified until it is watched on real Tank hardware.
