...
- **The TestFlight gate is a toolchain problem, not a CI one.** The Mac runner only has Xcode-beta 27.0, and App Store Connect rejects beta-built uploads. iOS builds and the full archive/upload pipeline already run locally on that Mac, so Xcode Cloud adds nothing and is red on every PR; it should be retired in App Store Connect. An untested route to ship sooner — build the distribution artifact with released Xcode 26.5, which the `canImport` guards and the iOS 18 deployment target may already allow — is written up under "Toolchain" in [`docs/CI_TROUBLESHOOTING.md`](CI_TROUBLESHOOTING.md). Xcode 27 beta stays the primary development toolchain; no iOS 27 capability should be dropped to make distribution easier.
