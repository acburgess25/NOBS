...
from app import research_tools
...
class ToolRegistry:
    ...
    _require_argument_keys = staticmethod(research_tools.require_argument_keys)

    # ------------------------------------------------------------------ #
    # Research tools                                                      #
    # ------------------------------------------------------------------ #
    # Implementations live in app/research_tools.py. They need nothing from
    # the registry but `settings`, so they stay plain functions there and the
    # registry keeps only the wiring.

    def _web_search(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return research_tools.web_search(self.settings, arguments)

    def _get_weather(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return research_tools.get_weather(self.settings, arguments)

    def _read_news_feeds(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return research_tools.read_news_feeds(self.settings, arguments)

    def _read_url(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return research_tools.read_url(arguments)

    def _lookup_wikipedia(self, arguments: dict[str, Any]) -> dict[str, Any]:
        return research_tools.lookup_wikipedia(arguments)
