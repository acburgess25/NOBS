// app-icon.jsx — NOBS app icon refinement (extends 09 direction)
// Amber rounded square + raw stamped N. Variations: scale, color, mark, in situ.
...
function NobsIcon({
  ...
  letter = 'N',
  filterStrength = 1,
  cornerScale = 0.21,    // iOS-ish squircle (close enough)
  letterScale = 0.74,    // slightly smaller so it sits cleanly above the scribble
  ...
  // Center the N in the area above the scribble (top 0.05 → ~0.72 of icon).
  const letterCx = vb / 2;
  const letterCy = vb * 0.40;
  return (
    <svg viewBox={`0 0 ${vb} ${vb}`} width={size} height={size}>
      ...
      <g filter={filterStrength > 0 ? `url(#${id}-marker)` : undefined}>
        <text x={letterCx} y={letterCy}
              ...
              fontSize={fs} fill={fg}
              letterSpacing="-6">{letter}{dot && <tspan fill={APAL.amberBright}>.</tspan>}</text>
      </g>
      {tick && (
        <g filter={filterStrength > 0 ? `url(#${id}-marker)` : undefined}
