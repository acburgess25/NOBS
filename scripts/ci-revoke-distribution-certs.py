#!/usr/bin/env python3
"""Revoke Apple Distribution certificates via App Store Connect API."""
...
def main() -> int:
    ...
    with httpx.Client(timeout=30.0) as client:
        for cert_type in ("IOS_DISTRIBUTION", "DISTRIBUTION", "MAC_APP_DISTRIBUTION"):
            response = client.get(
                f"{base}/certificates",
    ...
    if revoked == 0:
        print("No distribution certificates to revoke.")
    else:
        print(f"Revoked {revoked} distribution certificate(s).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Failed to revoke distribution certificates: {exc}", file=sys.stderr)
        raise
