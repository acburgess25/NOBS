...
enum NOBSColor {
    // ── Light ────────────────────────────────────────────────
    static let bgLight        = Color(hex: 0xF8FAFC)
    static let surfaceLight   = Color(hex: 0xFFFFFF)
    static let surfaceAltL    = Color(hex: 0xF1F5F9)
    static let textLight      = Color(hex: 0x0F172A)
    static let text2Light     = Color(hex: 0x64748B)
    static let text3Light     = Color(hex: 0x94A3B8)
    static let dividerLight   = Color(red: 15/255, green: 23/255,  blue: 42/255, opacity: 0.06)

    // ── Dark ─────────────────────────────────────────────────
    static let bgDark         = Color(hex: 0x0F172A)
    static let surfaceDark    = Color(hex: 0x1E293B)
    static let surfaceAltD    = Color(hex: 0x1E293B)
    static let textDark       = Color(hex: 0xF8FAFC)
    static let text2Dark      = Color(hex: 0x94A3B8)
    static let text3Dark      = Color(hex: 0x64748B)
    static let dividerDark    = Color(red: 248/255, green: 250/255, blue: 252/255, opacity: 0.05)

    // ── Brand (mode-agnostic) ────────────────────────────────
    static let nobsRose       = Color(hex: 0xFA5C5C)
    static let roseDeep       = Color(hex: 0xE11D48)
    static let roseSoft       = Color(hex: 0xFB7185)
    static let healthEmerald  = Color(hex: 0x10B981)
    static let emeraldDeep    = Color(hex: 0x059669)
    static let wellnessTeal   = Color(hex: 0x0D9488)
}
  ...
  return (
    <div style={{ background: c.bg, color: c.text, fontFamily: NOBS.font, padding: 48, height: '100%', boxSizing: 'border-box', overflow: 'auto' }}>
      ...
      <Block title="Primary button"
        preview={<PrimaryButton dark={dark} icon="send">Save log</PrimaryButton>}
        specs={[
          ['height',  '56pt (lg) · 48pt (md) · 36pt (sm)'],
          ['radius',  'pill (height / 2)'],
          ['padding', '0 28pt horizontal'],
          ['bg',      '$nobsRose · #FA5C5C'],
          ['shadow',  '0/3/10 rose@35% + 0/1/2 rose@25%'],
          ['typo',    'rounded 17 / .bold · letterSpacing -0.2'],
          ['hit',     '≥ 44pt always'],
        ]}
      />

      <Block title="Health log card"
        preview={<div style={{ width: 280 }}><MemoryCard dark={dark} tag="Diet" date="Just now" pinned body="Grilled salmon salad with avocado." /></div>}
        specs={[
          ['radius',  '22pt'],
          ['padding', '18pt'],
          ...
          ['tag',     'pill · 12/700 caps · diet|meds|wellness tint bg'],
          ['pinned',  'rose pin · top-right · 35° rotation'],
        ]}
      />

      <Block title="Medication row"
        preview={<div style={{ width: 280 }}><TaskRow dark={dark} title="Lipitor 20mg" due="9:00 PM" tag="Meds" priority /></div>}
        specs={[
          ['minHeight','52pt (touch target)'],
          ['padding', '14/16 inside list'],
          ['checkbox','24×24 · 2px border @ #D1C8BC / emerald fill done'],
          ['anim',    'spring 0.4 / 0.7 on check toggle'],
          ['strike',  'apply on done · text → textTertiary'],
          ['priority','rose flag · 16pt right-aligned'],
        ]}
      />
      ...
      <Block title="Tab bar"
        ...
        specs={[
          ...
          ['tint',    'rose active · textTertiary idle'],
  ...
  return (
    <div style={{ background: c.bg, color: c.text, fontFamily: NOBS.font, padding: 56, height: '100%', boxSizing: 'border-box', overflow: 'auto' }}>
      ...
      <Section title="2 · Color modes">
        ...
        NOBS Rose and Health Emerald are mode-agnostic — they shift slightly perceptually but use the same hex.
        Never use system blue for primary actions; nobs rose is the brand action color.
      </Section>
      ...
      <Section title="3 · Radii are big, intentionally">
        ...
        This is the single biggest knob that makes NOBS feel warm and premium rather than utilitarian.
      </Section>
      ...
      <Section title="5 · Privacy iconography">
        Emerald is reserved for &ldquo;done / safe / encrypted&rdquo;. Use it for: completed medication checkboxes, encryption indicators, the
        green switch state, and the &ldquo;synced&rdquo; light on the avatar.
        Don&rsquo;t use emerald as a CTA — that&rsquo;s rose&rsquo;s job.
      </Section>

      <Section title="6 · Composer">
        The compose bar is the single most-touched element. Always at the bottom of Health logs and Meds schedule, above the tab bar, with a 14pt drop shadow.
        Tapping it slides the keyboard up and reveals the AI suggestion chip (see <i>Add log</i> screen).
