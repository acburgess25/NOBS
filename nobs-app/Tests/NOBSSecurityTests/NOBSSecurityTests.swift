...
final class NOBSSecurityTests: XCTestCase {
    ...
    func testSaveAndLoadValue() throws {
        let store = KeychainStore(service: "com.nobs.test.\(UUID().uuidString)", inMemory: true)
    ...
    func testSeparateServicesDoNotCollide() throws {
        let storeA = KeychainStore(service: "com.nobs.test.A.\(UUID().uuidString)", inMemory: true)
        let storeB = KeychainStore(service: "com.nobs.test.B.\(UUID().uuidString)", inMemory: true)
