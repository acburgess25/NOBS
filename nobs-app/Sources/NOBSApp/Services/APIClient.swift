...
class APIClient: ObservableObject {
    ...
    func loginWithApple(userIdentifier: String, displayName: String) {
        let name = displayName.isEmpty ? "Apple User" : displayName
        let sentinel = "apple-\(userIdentifier)"
        self.token = sentinel
        self.username = name
        saveToKeychain(token: sentinel, username: name)
    }
    ...
    /// Execute a shell command on tank. iOS apps can't spawn subprocesses, so
    /// this POSTs to the tank API which runs the command server-side and
