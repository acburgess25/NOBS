...
public actor ConversationHistory {
    ...
    public func messages(from sender: String) throws -> [String] {
        try repository.search(query: sender).map(\.content)
    }
}
...
public actor iMessageHandler: IntentHandler {
    ...
    public func readHistory(from sender: String?) async throws -> String {
        ...
        let messages = try await history.messages(from: sender)
        if messages.isEmpty { return "No message history with \(sender)." }
