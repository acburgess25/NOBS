...
public final class NOBSDatabase: @unchecked Sendable {
    ...
    /// Personal mode is enabled for all users during beta.
    public var isPersonalModeEnabled: Bool { true }
    ...
    public func setup(storageMode: StorageMode = .localOnly, inMemory: Bool = false) throws {
        ...
        // If inMemory is true, we always reset and recreate the in-memory stores (important for tests)
        if inMemory {
            containers.removeAll()
        } else if !containers.isEmpty {
            // Already set up in production, check if we need to do anything
            // If all cases are present, we are good.
            if DataContext.allCases.allSatisfy({ containers[$0] != nil }) {
                return
            }
        }
        ...
        for context in DataContext.allCases {
            // Skip already loaded containers if not resetting
            if containers[context] != nil { continue }
    ...
    private func makeContainer(
        ...
        if let error = loadError {
            // In case of store migration, version hash mismatch, or other load errors on disk,
            // perform an automatic database recovery by deleting the incompatible store and creating a fresh one.
            if !inMemory {
                let nsError = error as NSError
                // Standard Core Data error codes related to version hash mismatch, migration errors, or corrupted stores.
                let isMigrationOrIncompatible = nsError.domain == NSCocoaErrorDomain &&
                    (nsError.code == NSMigrationError ||
                     nsError.code == NSMigrationMissingSourceModelError ||
                     nsError.code == NSMigrationMissingMappingModelError ||
                     nsError.code == NSMigrationManagerSourceStoreError ||
                     nsError.code == NSPersistentStoreIncompatibleVersionHashError ||
                     nsError.code == NSPersistentStoreSaveConflictsError)
                
                if isMigrationOrIncompatible {
                    print("⚠️ Incompatible database detected for \(dataContext.storeName) (code: \(nsError.code)). Performing automatic recovery...")
                    let storeURL = Self.storeURL(for: dataContext)
                    
                    // Attempt to cleanly remove the store files
                    ...
                    // Retry loading the persistent store
    ...
    static func storeURL(for dataContext: DataContext) -> URL {
        ...
        if let sharedContainer = fileManager.containerURL(forSecurityApplicationGroupIdentifier: "group.com.nobsdash.nobs") {
            dir = sharedContainer.appendingPathComponent("Library/Application Support/NOBS", isDirectory: true)
        } else {
            let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            dir = appSupport.appendingPathComponent("NOBS", isDirectory: true)
        }
    ...
    // Cached singleton — CoreData entity-to-class mappings are process-wide,
    // so recreating the model on every container reset causes conflicts.
    nonisolated(unsafe) static let sharedModel: NSManagedObjectModel = makeManagedObjectModel()
